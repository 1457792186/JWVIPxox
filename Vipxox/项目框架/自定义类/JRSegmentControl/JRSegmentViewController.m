...
- (void)viewDidLoad {
    ...
    [self setupViewControllersWithIndex:0];
...
- (void)setupViewControllersWithIndex:(NSInteger)index{
    if (![self.viewControllers[index] isKindOfClass:[UIViewController class]]) {
        Class vcClass = self.viewControllers[index];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (_isDrag) {
        ...
        [self setupViewControllersWithIndex:index];
